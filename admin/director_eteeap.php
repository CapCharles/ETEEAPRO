...
if ($_POST && isset($_POST['review_application'])) {
    ...
    $director_comments = trim($_POST['director_comments']);
    
    if (empty($director_comments)) {
    ...
    if (empty($errors)) {
        try {
            ...
            // Update application status and add director comments
            $stmt = $pdo->prepare("
                UPDATE applications 
                ...
                WHERE id = ?
            ");
            $stmt->execute([$new_status, $director_comments, $user_id, $app_id]);
...
$stats = [];
try {
    // Total applications submitted by faculty expert
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM applications WHERE application_status = 'faculty_submitted'");
    ...
    // Needs revision
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM applications WHERE application_status = 'needs_revision'");
    $stats['needs_revision'] = $stmt->fetch()['total'];
    
    // Rejected
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM applications WHERE application_status = 'director_rejected'");
...
try {
    ...
    switch ($filter) {
        case 'pending':
            $where_clause = "WHERE a.application_status = 'faculty_submitted'";
            break;
        case 'approved':
            $where_clause = "WHERE a.application_status = 'director_approved'";
            break;
        case 'revision':
            $where_clause = "WHERE a.application_status = 'needs_revision'";
            break;
        case 'rejected':
            $where_clause = "WHERE a.application_status = 'director_rejected'";
            break;
        default:
            $where_clause = "WHERE a.application_status IN ('faculty_submitted', 'director_approved', 'needs_revision', 'director_rejected')";
    }
...
<head>
    ...
    <style>
        ...
        .status-faculty_submitted { background-color: #fff3cd; color: #664d03; }
        .status-director_approved { background-color: #d1e7dd; color: #0f5132; }
        .status-needs_revision { background-color: #f8d7da; color: #721c24; }
        .status-director_rejected { background-color: #e9ecef; color: #495057; }
        .main-content {
            padding: 2rem;
        }
        .page-header {
        ...
        <div class="row">
            ...
            <nav class="col-md-3 col-lg-2 d-md-block sidebar">
                <div class="position-sticky pt-3">
                    <div class="text-center mb-4">
                        ...
                        <h5 class="text-white">Director ETEEAP</h5>
                    ...
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="director_eteeap.php?filter=revision">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 main-content">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h2 class="mb-1">
                                ...
                                Director ETEEAP Dashboard
                            </h2>
                            <p class="text-muted mb-0">Review and approve faculty expert submissions</p>
                        </div>
                        <div class="text-end">
                ...
                <div class="filter-tabs">
                    <a href="director_eteeap.php?filter=pending" class="btn btn-sm <?php echo $filter === 'pending' ? 'btn-primary' : 'btn-outline-primary'; ?>">
                        <i class="fas fa-clock me-1"></i>Pending
                    </a>
                    <a href="director_eteeap.php?filter=approved" class="btn btn-sm <?php echo $filter === 'approved' ? 'btn-success' : 'btn-outline-success'; ?>">
                        <i class="fas fa-check me-1"></i>Approved
                    </a>
                    <a href="director_eteeap.php?filter=revision" class="btn btn-sm <?php echo $filter === 'revision' ? 'btn-danger' : 'btn-outline-danger'; ?>">
                        <i class="fas fa-edit me-1"></i>Needs Revision
                    </a>
                    <a href="director_eteeap.php?filter=rejected" class="btn btn-sm <?php echo $filter === 'rejected' ? 'btn-secondary' : 'btn-outline-secondary'; ?>">
                        <i class="fas fa-times me-1"></i>Rejected
                    </a>
                    <a href="director_eteeap.php" class="btn btn-sm <?php echo empty($filter) || $filter === 'all' ? 'btn-dark' : 'btn-outline-dark'; ?>">
                            ...
                            <div class="application-card">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-3 text-end">
                                        ...
                                        <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#reviewModal<?php echo $app['id']; ?>">
                                ...
                                <?php if ($app['director_comments']): ?>
                                    ...
                                    <div class="alert alert-info mb-0">
                                        <strong><i class="fas fa-comment me-2"></i>Director's Comments:</strong>
                                        <p class="mb-0 mt-2"><?php echo nl2br(htmlspecialchars($app['director_comments'])); ?></p>
                                    </div>
                                <?php endif; ?>
                            </div>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <form method="POST">
                                            <div class="modal-body">
                                                ...
                                                <!-- Application Details -->
                                                <div class="mb-4">
                                                    <h6 class="border-bottom pb-2 mb-3">Application Details</h6>
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <p><strong>Submitted:</strong> <?php echo date('M j, Y', strtotime($app['updated_at'])); ?></p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="mb-4">
                                                    <h6 class="border-bottom pb-2 mb-3">Faculty Expert Recommendation</h6>
                                                ...
                                                <div class="mb-3">
                                                    <label class="form-label fw-bold">
                                                        <i class="fas fa-comment me-2"></i>Director's Comments *
                                                    </label>
                                                    <textarea class="form-control" name="director_comments" rows="4" required 
                                                              placeholder="Enter your comments and feedback here..."><?php echo $app['director_comments'] ?? ''; ?></textarea>
                                                    <small class="text-muted">Provide detailed feedback for the faculty expert and candidate</small>
                                                </div>
