...
function sendApprovalWithProgram($user_email, $user_name, $program_code, $program_name, $reroute_reason = '') {
    ...
    $preheader = "Your ETEEAP application has been approved. Program: {$program_code} - {$program_name}.";

    $reasonHtml = '';
    if (!empty($reroute_reason)) {
        $reasonHtml = '<div style="margin-top:8px;padding:10px 12px;border:1px solid #ffe8a1;border-radius:8px;background:#fff8e1;">
            <strong>Note:</strong> ' . nl2br(htmlspecialchars($reroute_reason)) . '
        </div>';
    }

    // (same head/styles as the “new template” you pasted earlier)
    // -- start of simplified body only to save space --
    $html = '<!DOCTYPE html><html lang="en"><head>...same styles as new template...</head>
<body style="background-color:#f3f5f9;margin:0;padding:0;">
  <div style="display:none;overflow:hidden;line-height:1px;opacity:0;max-height:0;max-width:0;">' . htmlspecialchars($preheader) . '</div>
  <table role="presentation" width="100%" bgcolor="#f3f5f9"><tr><td align="center" style="padding:24px;">
    <table role="presentation" width="600" style="max-width:600px;">
      <tr><td style="padding:24px;">
        <table role="presentation" width="100%"><tr>
          <td style="font:14px Arial;color:#344054;"><strong>ETEEAP System</strong></td>
          <td align="right" style="font:12px Arial;color:#98A2B3;">'.date('F j, Y').'</td>
        </tr></table>
      </td></tr>

      <tr><td style="padding:0 24px 24px 24px;">
        <table role="presentation" width="100%" style="background:#ffffff;border-radius:12px;overflow:hidden;">
          <tr><td align="center" style="background:linear-gradient(135deg,#22c55e 0%,#16a34a 100%);padding:24px;">
            <div style="font:700 22px Arial;color:#fff;">Application Approved</div>
            <div style="font:14px Arial;color:#eafff2;margin-top:4px;">Congratulations! You can now continue to the next steps.</div>
          </td></tr>

          <tr><td style="padding:24px;font:14px Arial;color:#344054;line-height:1.7;">
            ...
            <div style="margin:12px 0 14px 0;padding:12px 14px;border:1px solid #e6e8ee;border-radius:8px;background:#fbfcff;">
            ...
            <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:12px 0 4px 0;">
              <tr><td align="center" bgcolor="#3b82f6" style="border-radius:8px;">
                <a href="'.$baseUrl.'auth/login.php" style="display:inline-block;padding:12px 18px;font:600 14px Arial;color:#fff;background:#3b82f6;border-radius:8px;">Log in to your account</a>
              </td></tr>
            </table>

            <p style="margin:16px 0 0 0;color:#667085;font:12px Arial;">
          ...
          <tr><td style="padding:16px 24px 24px 24px;font:12px Arial;color:#667085;">Need help? Reply to this email or contact support.</td></tr>
        </table>
      </td></tr>

      <tr><td style="padding:0 24px 32px 24px;">
        <table role="presentation" width="100%" style="background:#0f172a;border-radius:12px;">
          <tr><td align="center" style="padding:18px 16px;font:12px Arial;color:#cbd5e1;">
            &copy; '.date('Y').' ETEEAP System. All rights reserved.
    ...
    error_log("ApprovalWithProgram email to {$user_email} => " . ($ok ? "Success" : "Failed"));
    return $ok;
}
